...
4. **Bridge removed.** Older code referenced `HeliographBridge` in `src/mcp/server.py`. Anything still importing it will fail. Use `src.mcp.registry.discover_tools()` and call tools directly.
