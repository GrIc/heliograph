...
Agent Hub exposes both transports, so no code change is required — only a
...
Use when Agent Hub runs as a long-lived service (Docker, systemd, etc.).
...
# Or one-shot:
opencode run "Use agent-hub to list the MCP tools available"
```

OpenCode lists discovered MCP tools at startup. Each Agent Hub tool appears
...
Use when you want OpenCode to spawn Agent Hub as a subprocess — no port, no
...
```json
{
  ...
  "mcp": {
    "agent-hub": {
      ...
      "enabled": true,
      "environment": {
        "PYTHONPATH": "/absolute/path/to/agent-hub"
      }
    }
  }
}
...
Requires: Python venv with Agent Hub deps installed, and `.env` set up at
...
| Symptom | Check |
|---------|-------|
| `agent-hub` tools missing from OpenCode startup banner | `opencode mcp list` — should show `agent-hub: connected`. If not, run `opencode --print-logs` and look for MCP init errors. |
...
- OpenCode supports both the older `command` array style and the newer
  HTTP/SSE remote transport. Agent Hub works with both — pick based on
  whether you want a service (SSE) or a subprocess (stdio).
- Citations returned by Agent Hub appear as plain JSON in the tool result;
  OpenCode's TUI renders them as clickable references when the path is
  recognized.
- For an OpenCode-specific permissions config (allow/deny per Agent Hub
  tool), use OpenCode's `permission` section — Agent Hub itself doesn't
