# Agent Hub — Pitch honnête pour devs
...
Agent Hub est un serveur MCP qui indexe une codebase (vector + graph + temporal) et expose ce savoir à des agents IA (Roo, Cursor, Claude Code, Continue) via des outils typés avec citations source-ligne obligatoires.
...
| Solution | Force | Faiblesse vs Agent Hub |
|---|---|---|
...
Agent Hub se positionne sur **MCP + grounding strict + auto-hébergement**. Si tu n'as besoin d'aucun des trois, autre chose est probablement mieux.
...
- **Si tu n'as pas de leur codebase indexée** : remplace par `find_code` sur agent-hub lui-même (méta : "il s'analyse lui-même").
...
> "Agent Hub, c'est un index 3-en-1 (vector + graph + temporal) avec un protocole standard (MCP) et un contrat de citation strict. C'est utile si vos agents IA répondent aujourd'hui n'importe quoi sur votre code. C'est inutile si vos devs préfèrent grep + lire."
